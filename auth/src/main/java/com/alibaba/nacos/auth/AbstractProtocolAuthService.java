...
public abstract class AbstractProtocolAuthService<R> implements ProtocolAuthService<R> {
    ...
    private final Logger LOGGER = LoggerFactory.getLogger(AbstractProtocolAuthService.class);
    ...
    @Override
    public boolean validateAuthority(String username, Permission permission) throws AccessException {
        LOGGER.info("authConfigs.getNacosAuthSystemType() => " + authConfigs.getNacosAuthSystemType());
        Optional<AuthPluginService> authPluginService = AuthPluginManager.getInstance()
                .findAuthServiceSpiImpl(authConfigs.getNacosAuthSystemType());
        if (authPluginService.isPresent()) {
            LOGGER.info("authPluginService.get() => " + authPluginService.get());
            return authPluginService.get().validateAuthority(username, permission);
        }
        return true;
    }
